...
class ReplayBuffer:
    ...
    def __init__(self, capacity, alpha, state_dim, action_dim):
        """
        ### Initialize
        """
        ...
        # Arrays for buffer
        self.data = {
            'obs': np.zeros(shape=(capacity, state_dim), dtype=np.float32),
            'action': np.zeros(shape=(capacity, action_dim), dtype=np.float32),
            'reward': np.zeros(shape=capacity, dtype=np.float32),
            'next_obs': np.zeros(shape=(capacity, state_dim), dtype=np.float32),
            'not_done': np.zeros(shape=capacity, dtype=bool),
            'omega': np.zeros(shape=(capacity, 2), dtype=np.float32)
        }
    ...
    def add(self, obs, action, next_obs, reward, done, omega):
        """
        ### Add sample to queue
        """
    ...
    def sample(self, batch_size, beta):
        ...
        return (
            torch.FloatTensor(samples['obs']).to(self.device),
            torch.FloatTensor(samples['action']).to(self.device),
            torch.FloatTensor(samples['next_obs']).to(self.device),
            torch.FloatTensor(samples['reward']).to(self.device),
            torch.FloatTensor(samples['not_done']).to(self.device),
            torch.FloatTensor(samples['omega']).to(self.device),
            torch.FloatTensor(samples['weights']).to(self.device),
            torch.IntTensor(samples['indexes']).to(self.device)
        )
